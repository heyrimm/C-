...
//void는 비어있는 또는 아무 것도 없는 의미를 가지며, 함수나 포인터를 정의할 떄 사용한다.
...
//2. 포인터로서의 void
// 포인터를 선언할 때 데이터 타입으로 void를 사용하면 어떤 데이터 타입의 메모리 
// 주소든 포인터 변수에 할당할 수 있다.

// void 포인터는 포인터 연산이 불가능. 그래서 메모리의 내용을 읽거나 쓰기 위해서는 적절한 데이터 타입으로
//캐스팅(형변환)해줘야한다.
...
//	void* ptr;
//	ptr = &num;
//	printf("num = %d\n", *(int*)ptr);//맨 앞의 *은 역참조 (int *)는 형 변환
//
//	ptr = &fnum;
//	printf("num = %f", *(float*)ptr);//맨 앞의 *은 역참조 (int *)는 형 변환
//}

// Static 변수

// static 이라는 키워드를 사용하여 static 변수를 선언할 수 있다. static은 전역 변수와 지역 변수에 모두 선언 가능.
// 지역 변수를 일부 보완해주는 역할을 한다.
...
// 함수에서 배열을 전달받을 떄는 배열의 이름을 그대로 함수에 인자로 전달한다.
// 이때 배열의 이름은 '첫번째 요소의 주소를 나타낸다.' 함수에서는 전달받은 배열의 첫번째 요소를 가리키는
// 포인터 변수를 선언하여 사용한다.
...
// 팩토리얼 : 자기를 포함해서 하나씩 작은 수를 곱해가면서 1까지 곱해나가는 것
...
//재귀함수를 사용, 자연수 n을 입력받아 1부터 n까지 합을 구하는 sum을 재귀호출로 구현
//#include<stdio.h>
//int sum(int n)
//{
//	if (n > 1)
